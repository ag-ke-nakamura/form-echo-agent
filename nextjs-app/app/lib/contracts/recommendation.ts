 ...
 * 参加可否表から人数を数える。#109（ADR-0011）で `contracts/recommendation.ts` から
 * 複製した、nextjs-app 専用の実装（他プロジェクトはこのロジックを持たない）。
 *
 ...
 * すべてここが参加可否表から数える。数えた結果を返させると、表を見れば分かることを
 * モデルが間違える余地が残る。
 */

import type { RecommendScheduleInput, RecommendScheduleOutput } from "./types";
import type { Availability } from "./meeting";
 ...
 *
 * `meeting.ts` の列挙と同じく**配列を正典にして型を導く**。
 */
...
/**
 * 評点からラベルを決める下限。各値は**その値以上**を意味する（`recommended` は
 * 0.80〜1.00）。どれにも届かなければ `rejected` になる。
 */
...
/**
 * 候補日程ひとつを数える。**名簿を主にして数える。** 回答の側から数えると、名簿に
 * 無い参加者の回答が混ざったときに「参加可能5名／名簿4名」のような表が出る。
 *
 * 参加可否4状態を1つの `switch` で網羅し、参加可能人数を**内訳の和**として出す —
 * 現地とリモートの内訳はどのみち数えるので、「出席は2通りある」という事実がこの
 * `switch` に一度だけ現れる形になる。状態が増えれば `switch` の網羅性検査が漏れを指す。
 */
 ...
 *
 * 同点は日時が早いほう、それも同じなら現地で参加できる人数が多いほうで捌く。
 * **どちらの観点も AI には訊かない**。訊けば評点と食い違う組が返る。
 */
