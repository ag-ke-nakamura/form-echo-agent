...
- `reservation-form.ts` — 交通ICタブの状態と組み立て（#38・#65・#167）。欄の表示名・
  ...
  **同行者とICカード利用枚数もここにあるが `fields` の外** — 欄として写す対象では
  ないため（#176。同行者は**行の有無**で守り、利用枚数は行数から**導く**）、中に
  入れると欄の写す規則が掛かるものに見える）・**状態の遷移すべて**（手入力
  `setFieldValue`・同行者の行の足し引き・利用枚数の上書き `setCardCount` と導出
  `cardCount`・初期状態 `EMPTY_RESERVATION` と「最初からやり直す」`resetReservation`。
  リセットは行番号だけ持ち越す — 消した行の番号を再利用すると、入力中の氏名が別の行へ
  移る）・**AI の結果をタブの状態へ写す `applyToReservation`** とプレビューの一覧
  （**同行者の人数は出力契約に載るが氏名は載らない**。AI が作るのは空の行までで、
  行が1つでもあれば触らない）
...
- `ai-assistant/field-source.tsx` — 値の出どころ（`"default"` / `"manual"` / `"ai"`）の印と、
  ...
  置いた値**（空の初期状態、プレプリント）で、上書きの可否を `manual` かどうかだけで決める
  ため（ADR-0018）。**`"default"` を生成するのは交通ICだけ** — 会議3タブは初期状態を持たず
  `null` か未登録で表す
...
| タブ | 守る単位 | 理由 |
| --- | --- | --- |
| 交通IC | 欄 | 欄が固定なので1対1で対応が付く。守るのは `source === "manual"` の欄だけ（初期状態は `"default"`） |
...
| 参加可否回答 | 候補日程（参加可否と備考を別々に） | 手で選んだ可否は本人の予定そのもの。備考は印が参加可否側にしか無いので出どころを別に持つ（持たないと保護から漏れる）。どちらも守るかは出どころだけで決める — 「消して空にした」のは参加者の意図（#179） |
...
変わらないので、錠の印と「手入力のため変更しません」で分ける（判定は `applyToReservation` /
...
**「守る」の判定は出どころだけを見る（空かどうかは見ない）。** 交通ICのフォームの初期状態は
`"default"` なので、「空で `manual`」は職員が「消す」で空にした欄しか指さない。消した欄は
空のまま残り、報告の「守った」側に出る（#166・ADR-0018）。
...
**`CONTEXT.md` は概念の名前を決める場所で、画面の文言の規定ではない。** 用語と職員が読む
ラベルはずれてよい（欄名「1人あたり運賃」は用語「1人あたり運賃（見込み）」のラベルで、
「（見込み）」を落としている）。**ただし用語の `_Avoid_` に挙がっている語を画面に出すのは
別**で、あれはその語で呼ぶと概念が壊れると決めた語である。
