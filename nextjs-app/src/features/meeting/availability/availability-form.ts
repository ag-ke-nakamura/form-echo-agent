...
/*
  候補日程の型だけ候補日程画面から引く（画面を跨ぐが同じ feature の内側なので相対）。
  **`import type` なので実行時には消え**、この module が候補日程画面を読み込むことは
  ない（vitest も同じ）。同じ形をここで書き直すと、入力契約に欄が増えたときに
  型検査がこちらまで届かなくなる。
*/
...
import type { Availability, MeetingFormat } from "@/lib/contracts/meeting";
import { AVAILABILITY_ORDER, isAttending } from "@/lib/contracts/meeting";
import { candidateLabel } from "../shared/meeting-info";
 ...
 *
 * 値域そのものは `@/lib/contracts/meeting.ts` にある。ここに残るのは画面だけが要るもの —
