import type { ParseAvailabilityInput } from "@/lib/contracts/types";
import type { FieldSource } from "@/components/ai-assistant/field-source";
 ...
 * タブ側（`candidates-panel.tsx`）に置くと、この純関数の側から掘りに行くことになる。
 */
...
/** 升目の刻み（分）。`DURATION_OPTIONS` が30分刻みなのと同じ約束（`@/lib/contracts/meeting.ts`）。 */
 ...
 *
 * 起点を決めるのはマウント後（`use-candidate-calendar.ts`）。SSG なのでビルド時に描いた
