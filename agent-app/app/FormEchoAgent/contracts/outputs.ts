...
const routeCandidateSchema = z.object({
   ...
   *
   * **数値で持つ**（#169）。文字列だと「約2000円」「1980円（往復）」が通り、往復区分が
   * 往復なのに片道の額が入っていることを職員が目で確かめられない。単位は画面が足す。
   *
   * `min(0)` は無料区間を許して負の額を弾く線。0円の経路はありうるが、負の運賃は無い。
   */
  fare: z
    .number()
    .int()
    .min(0)
    .describe(
      'IC運賃の額。**単位・記号を付けず0以上の整数**で返す（例: 1980。「約1980円」「1,980円」は契約が弾く）。与件の往復区分が round なら往復分（片道の2倍）',
    ),
  ...
  /**
   * この候補を引いた経路検索結果の出典番号（#174、ADR-0019。`CONTEXT.md`「経路検索結果」）。
   *
   * WHY 候補ごとに持つか: 「1円でも安い経路を選んだ」という判断の根拠は候補ごとの
   * 検索結果なので、**採用理由を職員が確かめられるのは候補と根拠のページが対に
   * なっているときだけ**である。応答全体の `sources` では、どの候補の運賃がどの
   * ページの額なのかを言えない。
   *
   * **URL ではなく番号にした理由は ADR-0019。** URL を書き写させると、写し間違い・
   * 別経路の URL の合成・表記の揺れが起きる。番号なら渡されたものから選ぶしかない。
   *
   * **上限をここでは縛れない。** 番号の上限はそのリクエストが取得した出典の件数で、
   * 入力にも出力にも無い（検索と同時に増える）。範囲外の番号は画面が「確認できません
   * でした」と出し、Runtime が `log.warn` に残す — **応答を捨てない**（番号1つの
   * ために日付・目的・同行者・経路・運賃まで失うのは釣り合わない）。
   */
  citation_number: z
    .number()
    .int()
    .min(1)
    .describe(
      'この経路を読み取った検索結果の citation_number。web_search の結果に付いている番号をそのまま入れる。URL は書かない',
    ),
  /**
...
export const parseReservationOutputSchema = z
  .object({
    ...
    */
     ...
     * 返す日時とは別の概念である**（あの2つはカードの貸借の話で移動の話ではない）。
     *
     * **運賃・経路の計算には使わない。** 参照ドキュメントのプロンプトサンプルが
     * 「利用日時は経路・運賃計算の条件としては用いず、原則として現行の通常運賃・
     * 平日ダイヤを前提として計算する」と明記している（運賃の再現性を取る判断で、
     * 同じ申請を後から検証したときに額が変わらない）。**だから与件には載せない** —
     * 額を決めない値を `input` に足すと、ADR-0017 が引いた「運賃の額を決める与件だけ」
     * の線が消える。出力にだけ載るのは、追加指示の日時を AI が読み取って画面へ
     * 写すためである。
     *
     * **15分刻みはここで縛らない**（#175）。刻みは職員が選ぶときの画面の制約で、
     * 10:07 発でも経路は引ける。契約で強制すると、刻みを無視するブラウザや AI の
     * 読み取りで BFF が 400 を返す。
     */
    depart_at: isoDateTimeSchema
      .nullable()
      .describe(
        '出発日時（移動を始める日時）。YYYY-MM-DDTHH:mm 形式。ICカードを借りる日・返す日時とは別の項目。読み取れない場合は null',
      ),
    /**
