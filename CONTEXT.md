...
**移動経路**:
ICカード予約における出発地から目的地までの交通ルート（例:「新宿(東京メトロ丸ノ内線) => 霞ケ関(東京メトロ日比谷線) => 虎ノ門ヒルズ」）。区間ごとの構造は持たず、AI が整形した1本の文字列として持つ。
...
**定期重複区間**:
経路候補ひとつのうち、**定期区間**と重なる駅間。連続区間ごとに数える。自由文に**定期区間**の言及が無ければ区間そのものが不明なので、候補との重複が無いのとは区別して null で持つ。
