...
**該当**: 共通設計方針書 7.1節「9月スコープでの利用」

**事実**（AI開発担当への確認による）:

7.1節は Websearch の利用機能として3つを挙げている。

| 7.1節の記載 | 実際 |
| --- | --- |
| #977（交通IC経路探索）: 交通経路検索 | **必要** |
| #979（候補日程設定）: 会議室空き状況の確認（将来） | **不要** |
| #981（候補日提案）: 参加者の勤務地・交通アクセス情報 | **不要** |

Websearch を入れる理由は**経路情報の精度**である。経路・所要時間・列車名をモデルの内部知識だけで答えさせると、情報が古かったり存在しない便を答えたりする。検索で裏を取ることが要件になるのは、この性質を持つ交通ICだけ。

会議ロジ側の2機能（`parse-candidates` / `parse-availability` と推薦系）は自然文の解釈と候補の生成であり、外部の最新情報を必要としない。#979 には元から「（将来）」と付いているが、#981 は無条件に挙げられている。

**修正案**: 7.1節の一覧を #977 のみに絞り、Websearch が要る理由（経路情報の精度を内部知識で担保できない）を書く。#979 と #981 の行は落とす。
