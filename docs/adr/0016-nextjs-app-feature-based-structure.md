...
- `src/features/**` → `**/features/**` と `**/app/**` を禁止（`@/features/...` `@/app/...` の
  側を捕まえる）。ルート層も禁じるのは、依存を feature → 共有層の一方向に保つため
  （app → feature は通る。組み立てはあちらの仕事である）
- **あわせて「feature の外へ登ること自体」を禁止**（#158 で判明）。`**/features/**` は
  文字列を見るルールなので、隣の feature へ相対で登った `../../ic-card/x` を捕まえられない
  — その文字列に `features` が現れないからである。**何段登れば外に出るかは自分の深さで
  決まる**ので、深さごとにブロックを分ける（feature 直下 → `../**`、会議の画面の下 →
  `../../**`。`../shared/` は同じ feature 内なので通る）
- **深さの列挙には底を張る**（#158 で判明）。深さは列挙した分しか書けず、`src/features/**`
  に1段掘り足した瞬間にどのブロックにもマッチしなくなり、線が黙って消える。
  `src/features/**` 全体に「3段以上登る import」の禁止を敷いておく。
  **誤検知は落ちて分かるが、取りこぼしは静かに通る** — 倒す向きはこちらである
- `src/components/**` `src/lib/**` → `**/features/**` と `**/app/**` を禁止
- **発火することをテストで固定する**（#158）。`pnpm run lint` が言えるのは「違反が無い木で
  通った」ことだけなので、ESLint を直接呼んで違反を食わせるテストを `pnpm run test` に置く。
  glob を1つずつ潰して**すべてが落ちること**を実装中に確かめた（16/16）
