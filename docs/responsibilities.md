...
契約は3チームがそれぞれ自分のパッケージ内に複製として持つ（ADR-0011）。共有ディレクトリは無い。
...
**同じ検査が2度出てくるのは重複ではない。** BFF と Runtime がそれぞれ独立に持つ `outputSchemaFor` 相当を両方引くことで、どちらか片方だけが契約の変更に追随していない状態を実運用で検知する（ADR-0011）。禁じているのは**同じプロジェクト内で判断を書き写すこと**であって、同じ関数を呼ぶことではない。

## 5. 入出力契約 — 複製元は同じ、変更は手で3箇所へ

3プロジェクトがそれぞれ複製として持つ表（ADR-0011）。共有ディレクトリは無いので、**「同じ判断を2箇所に書かない」はプロジェクト内でだけ効く** — プロジェクトをまたぐ二重実装（Runtime と BFF がそれぞれ持つ `outputSchemaFor` 相当）は正式な設計として受け入れている。

| ファイル相当 | 決めること | 持つプロジェクト |
| --- | --- | --- |
| `task-ids.ts` | 受け付ける taskId とドメインの切り出し | 3プロジェクト全部 |
...
| 出力に欄を1つ足す | AI agent（自分の複製を直す）→ frontend（自分の複製を直す） | BFF は自分の複製も直して再検査を通す |
| 画面に渡す与件を1つ足す | frontend（自分の複製を直す）→ AI agent（自分の複製を直す） | BFF は素通し |
...
CI で回すコマンドは `CLAUDE.md`「変更を出す前の確認」の表にある。**入出力の形を変える変更は複製先すべてのプロジェクトを回す** — 自分のプロジェクトのぶんだけ回して済ませると、直し忘れた複製が実運用まで気付かれない。
