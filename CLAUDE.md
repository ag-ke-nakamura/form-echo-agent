...
**例外は `nextjs-app` ↔ `hono-app` の1辺だけ**（ADR-0015 がこの1辺に限って ADR-0011 を
改訂した）。詳細は `.claude/rules/contracts.md`。
...
**日本語を基本言語**とする。**会話での返答（チャットの本文）を含め**、コミットメッセージ、コードコメント（WHY を書く。WHAT は不要）、Markdown ドキュメント、ADR・仕様書すべて。スキルがテンプレートを提供する場合はそのフォーマットに従いつつ日本語で書く。英語のままでよいのは外部ライブラリの API 名・定数のみ。
...
| `infra` | `npx prettier --check . && npm run build && npm run test && npx cdk synth` |
...
**チケットの実装は必ずブランチを切ってから始める。mainに直接コミットしない。** `implement` スキル自身は「現在のブランチにコミットする」としか言わない（ブランチを切る判断を持たない）ので、**呼ぶ前に私がブランチを切る**。過去のPR履歴（`gh pr list --state all`）を見ると #59〜#84 相当のチケットはすべてこの形で、コミットメッセージの `(#68) (#82)` のような二重の番号（issue番号・PR番号）がその跡である。直近数コミットが素通りでmainに入ったのは意図した簡略化ではなく流れてしまっただけなので、ここに書いて固定する。

- **ブランチ名**: `{issue番号}-{英語の短い説明}`（例: `68-ic-card-datetime-and-purpose`）。過去の慣習をそのまま踏襲する
- **実装が終わったら PR を作るところで止める。** コミット後、`commit-commands:commit-push-pr` などでブランチを push して PR を作る。**squash merge は人間が GitHub 上で判断して実行する** — 私はマージまで自動でやらない
- 適用対象は issue に紐づく実装作業。ADR・CLAUDE.mdの棚卸しのような単発の小さな変更まで一律に縛る趣旨ではないが、**迷ったらブランチを切る**（過去はそういう変更もPRを通っている）
