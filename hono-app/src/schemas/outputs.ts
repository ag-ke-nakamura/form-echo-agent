...
const routeCandidateSchema = z.object({
  ...
  /**
   * この候補を引いた経路検索結果の出典番号（#174、ADR-0019）。**必須。**
   *
   * 採用理由を職員が確かめられるのは候補と根拠のページが対になっているときだけで、
   * 応答全体の `sources` では、どの候補の運賃がどのページの額なのかを言えない。
   *
   * **上限は縛らない。** 番号の上限はそのリクエストが取得した出典（`citations`）の
   * 件数で、入力にも出力にも無い。範囲外は画面が「確認できませんでした」と出す —
   * **BFF はここで応答を捨てない**（番号1つのために応答全体を失うのは釣り合わない）。
   */
  citation_number: z
    .number()
    .int()
...
export const parseReservationOutputSchema = z
  .object({
    ...
    */
     ...
     * 返す日時とは別の概念である（あの2つはカードの貸借の話）。
     *
     * **運賃・経路の計算には使わない**ので与件には載らない（ADR-0017 の線は
     * 「運賃の額を決める与件だけ」）。**15分刻みも縛らない** — 刻みは画面だけの
     * 制約で、10:07 発でも経路は引ける（#175）。ここで縛ると、刻みを無視する
     * ブラウザや AI の読み取りで再検査が `PARSE_FAILED` を返す。
     */
     ...
     * あるため。与件が既定値のままなら AI が勝ち、職員が手で選んでいたら AI は
     * 直さず聞き返す（ADR-0018。規則は Skill が持つ）。
     */
 ...
 *
 * `message` / `sources` を持たないのは、この taskId が **Structured Output を通らない**
 * ため — モデルはスキーマを見ておらず、Runtime が素のテキストをこの形に包んで返す。
 * 包む先が1欄だけなら、モデルが書かなかった欄を Runtime が捏造する余地が無い。
 */
